                ...
                <form action="{{ route('admin.items.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-2 flex-wrap">
                            @php
                                $catIcons = [
                                    // Guns: target / pistol-inspired icon (polished)
                            ...
                            @foreach($categories ?? [] as $cat)
                                @php
                                    // check for local custom icon files in public/storage/categories/{slug}.{svg,png}
                                    $localIcon = null;
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span id="item-image-filename" class="text-sm text-app-muted">No file chosen</span>
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="hidden" name="rarity" value="{{ old('rarity', 'common') }}">
                        @php
                            $rarities = [
...
<script>
    (function(){
        // rarity button group behavior: keyboard accessible, updates hidden input and preview badge
        ...
        buttons.forEach(btn => {
            // init appearance based on aria-checked
            if(btn.getAttribute('aria-checked') === 'true') setSelected(btn);
        ...
        // category radio badge behavior + preview update
        document.querySelectorAll('label.cursor-pointer').forEach(function(label){
            label.addEventListener('click', function(e){
                const radio = label.querySelector('input[type="radio"]');
                if(radio){
                    radio.checked = true;
                    // update visual ring on selected badge
                    document.querySelectorAll('label.cursor-pointer > span').forEach(function(span){
                        span.classList.remove('ring-2','ring-app-accent');
                    });
                    const badge = label.querySelector('span');
                    if(badge) badge.classList.add('ring-2','ring-app-accent');
