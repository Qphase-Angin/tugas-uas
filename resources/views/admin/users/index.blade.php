...
<div class="container-fluid pt-2" x-data="{ showDeleteModal: false, confirmDelete: {} }">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-white mb-1">Users List</h1>
            <p class="text-app-muted text-sm">Manage All System Users</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.users.create') }}" class="px-4 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-lg font-medium transition-all flex items-center gap-2">
                <i class="fas fa-user-plus"></i> Create User
            </a>
            <a href="{{ route('admin.users.active') }}" class="px-4 py-2.5 bg-app-accent hover:bg-app-accent/90 text-white rounded-lg font-medium transition-all flex items-center gap-2">
    ...
    <div class="bg-app-card border border-app-border rounded-xl overflow-hidden shadow-lg">
        ...
        <div class="flex items-center justify-between gap-4 p-4 border-b border-app-border bg-app-bg">
            <div class="flex items-center gap-2">
                ...
                <form method="GET" action="{{ route('admin.users.index') }}" class="inline-flex items-center gap-2">
                ...
                <thead class="bg-app-bg border-b border-app-border">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider cursor-pointer hover:text-white">
                            <a href="{{ route('admin.users.index', ['sort' => 'id', 'order' => request('sort') === 'id' && request('order') === 'asc' ? 'desc' : 'asc', 'per_page' => request('per_page'), 'search' => request('search')]) }}" class="flex items-center gap-2">
                                ID
                                @if(request('sort') === 'id')
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider cursor-pointer hover:text-white">
                            <a href="{{ route('admin.users.index', ['sort' => 'nama', 'order' => request('sort') === 'nama' && request('order') === 'asc' ? 'desc' : 'asc', 'per_page' => request('per_page'), 'search' => request('search')]) }}" class="flex items-center gap-2">
                                Nama
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider cursor-pointer hover:text-white">
                            <a href="{{ route('admin.users.index', ['sort' => 'email', 'order' => request('sort') === 'email' && request('order') === 'asc' ? 'desc' : 'asc', 'per_page' => request('per_page'), 'search' => request('search')]) }}" class="flex items-center gap-2">
                                Email
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider cursor-pointer hover:text-white">
                            <a href="{{ route('admin.users.index', ['sort' => 'role_id', 'order' => request('sort') === 'role_id' && request('order') === 'asc' ? 'desc' : 'asc', 'per_page' => request('per_page'), 'search' => request('search')]) }}" class="flex items-center gap-2">
                                Role
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider cursor-pointer hover:text-white">
                            <a href="{{ route('admin.users.index', ['sort' => 'is_active', 'order' => request('sort') === 'is_active' && request('order') === 'asc' ? 'desc' : 'asc', 'per_page' => request('per_page'), 'search' => request('search')]) }}" class="flex items-center gap-2">
                                Status
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider cursor-pointer hover:text-white">
                            <a href="{{ route('admin.users.index', ['sort' => 'alamat', 'order' => request('sort') === 'alamat' && request('order') === 'asc' ? 'desc' : 'asc', 'per_page' => request('per_page'), 'search' => request('search')]) }}" class="flex items-center gap-2">
                                Address
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold text-app-muted uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-app-bg/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex gap-2">
                                    ...
                                    <a href="{{ route('admin.users.edit', $user) }}" class="px-3 py-1.5 bg-app-accent/20 hover:bg-app-accent/30 text-app-accent rounded text-xs font-medium transition-all flex items-center gap-1">
    ...
    <div class="mt-6 flex items-center justify-between">
        <div class="text-sm text-app-muted">
            Showing <span class="text-white font-bold">{{ $users->firstItem() }}</span> to
            <span class="text-white font-bold">{{ $users->lastItem() }}</span> of
